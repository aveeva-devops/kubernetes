...
```
+
. Create S3 bucket: `aws s3api create-bucket --bucket kubernetes-devops-me --region us-west-2`
. Enable bucket versioning: `aws s3api put-bucket-versioning --bucket kubernetes-devops-me --region us-west-2 --versioning-configuration Status=Enabled`
. Set S3 bucket: `export KOPS_STATE_STORE=s3://kubernetes-devops-me`

==== Configure One master and two nodes Cluster on AWS

. Set cluster name: `export NAME=cluster.k8s.devops`
